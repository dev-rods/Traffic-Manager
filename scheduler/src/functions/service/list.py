...
def handler(event, context):
    """
    Handler para listagem de servicos de uma clinica via API.

    GET /clinics/{clinicId}/services
    Retorna todos os servicos ativos da clinica ordenados por nome.
    """
    try:
        ...
        logger.info(f"Listando servicos da clinica: {clinic_id}")

        # 3. Buscar servicos ativos da clinica
        db = PostgresService()
        ...
        services = [_serialize_row(row) for row in rows]
        ...
        error_msg = str(e)
        logger.error(f"Erro ao listar servicos: {error_msg}")
